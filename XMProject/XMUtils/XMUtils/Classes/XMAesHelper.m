...
//  XMUtils
//
//  Created by liu can on 2023/7/19.
//
//
//   ┏┛ ┻━━━━━┛ ┻┓
//   ┃　　　　　　 ┃
                   ...
                   padding:(XMCryptorPadding)padding;
{
    ...
    data = [self XMAESWithData:data operation:kCCEncrypt mode:mode key:key keySize:keySize iv:iv padding:padding];
    return [XMGTMBase64 stringByEncodingData:data];
}
...
+ (NSString *)XMAESDecrypt:(NSString *)originalStr
                   ...
                   padding:(XMCryptorPadding)padding
{
    NSData *data = [XMGTMBase64 decodeData:[originalStr dataUsingEncoding:NSUTF8StringEncoding]];
    data = [self XMAESWithData:data operation:kCCDecrypt mode:mode key:key keySize:keySize iv:iv padding:padding];
    return [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
}

+ (NSData *)XMAESWithData:(NSData *)originalData
                   ...
                   padding:(XMCryptorPadding)padding
{
    ...
    bytesTotal += bufused;
    if (padding == XMCryptorPKCS7Padding) {
...
+ (NSData *)removeBitPaddingWithData:(NSData *)sourceData mode:(CCMode)mode operation:(CCOperation)operation andPadding:(XMCryptorPadding)padding
{
    int correctLength = 0;
    ...
    }else if (padding == XMCryptorZeroPadding && end == 0) {
    ...
    }else if ((padding == XMCryptorANSIX923 || padding == XMCryptorISO10126) && (end > 0 && end < blockSize + 1)){
        correctLength = (short)sourceData.length - end;
//        if (padding == XMCryptorISO10126 || ( testByte[sourceData.length - end] == 0 && testByte[sourceData.length - 2] == 0)) {
//            correctLength = (short)sourceData.length - end;
//        }
    }
